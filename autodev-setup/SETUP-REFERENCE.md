...
- **No external helper skills.** Grilling, refactor target-finding, and production QA are all **bundled with autodev** — the `grill` skill (`templates/grill/`, installed to the project's `.claude/skills/grill/`) and the `/refactor-pass` + `/bug-hunt` commands themselves. The project never relies on a skill from a contributor's personal global collection. (Do not install or substitute someone's own grilling/architecture/QA skills.)
